...
namespace TestJuniorGF
{
    public partial class GameWindow : Window
    {
        ...
        {
            _buttons[buttonIndex].Background = new SolidColorBrush(Colors.DarkRed);
        }

        public void MarkDeselected(Vector2 buttonIndex)
        {
            Color color = ((buttonIndex.X + buttonIndex.Y) % 2 == 0) ? Colors.Gray : Colors.LightGoldenrodYellow;
        ...
        private void CreateGridLayout()
        {
            ...
            for (int i = 0; i < GridSize; i++)
            {
                for (int j = 0; j < GridSize; j++)
                {
                    Color color = ((i + j) % 2 == 0) ? Colors.Gray : Colors.LightGoldenrodYellow;
